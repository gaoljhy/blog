...
> 这个过程有点类似于，公司部门里，在使用着打印机打印东西的同时（还没有打印完），别人刚好也在此刻使用打印机打印东西，如果不做任何处理的话，打印出来的东西肯定是错乱的。

- 在线程里也有这么一把锁 —— 互斥锁（mutex）

互斥锁是一种简单的加锁的方法来控制对共享资源的访问，互斥锁只有两种状态,即**上锁**( lock )和**解锁**( unlock )。
