...
## 0x00 前言
   现在的网络环境往往是WAF/IPS/IDS保护着Web 服务器等等，这种保护措施往往会过滤挡住的SQL注入查询链接，甚至封锁的主机IP，所以这个时候，就要考虑怎样进行绕过，达到注入的目标。因为现在`WAF/IPS/IDS`都把sqlmap 列入黑名单
   
   但是本文基于`sqlmap` 进行绕过注入测试，介绍至今仍然实用有效的技巧，以下策略在特定的环境能够成功绕过，具体是否绕过，请仔细查看输出的信息。

-------
## 0x01 确认WAF
     首先判断该`Web` 服务器是否被`WAF/IPS/IDS`保护着。这点很容易实现，
     因为在漏扫或者使用专门工具来检测是否有`WAF`，这个检测，在`nmap` 的`NSE`，或者`WVS`的策略或者`APPSCAN`的策略中都有，可以利用这些来判断
...
5 建议：
      ...
      鉴于参数和32种脚本，在平时的注入，这些通过不同的多重组合来进行测试，这个测试或者比较耗时
