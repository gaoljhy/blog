...
`nohup nmap -p 端口 -script 数据库类型-brute.nse -script-args userdb=/root/username.txt,passdb=/root/password.txt -T 5 -v -iL alllive.txt > mssql_burte.txt &`
...
`nohup nmap  -p 1433 --script ms-sql-brute.nse -v -iL alllive.txt > ms-sql-brute.txt &`
